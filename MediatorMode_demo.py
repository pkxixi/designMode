...
class HousingAgency:
    ...
    def getSearchCondition(self, description):
        """这里有一个将用户描述信息转换成搜索条件的逻辑，为节省时间直接返回。"""
        return description

    def getMatchInfos(self, searchCondition):
        """根据房源信息的各个属性查找最匹配的信息"""
        print(self.getName(), "为您找到如下最适合的房源：")
        for info in self.__houseInfos:
            info.showInfo(False)
    ...
    def signContracts(self, period):
        for info in self.__houseInfos:
            self.signContract(info, period=period)


class HouseOwner:
    def __init__(self, name, address):
        self.__name = name
        self.__houseAddress = address
    ...
    def setHouseInfo(self, address, area, price, hasWindow, bathroom, kitchen):
        self.__houseInfo = HouseInfo(
            area, price, hasWindow, bathroom, kitchen, address, self
        )

    def publishHouseInfo(self, agency):
        agency.addHouseInfo(self.__houseInfo)
        print(self.getName() + "在", agency.getName(), "发布房源信息：")
